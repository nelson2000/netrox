...
resource "aws_elastic_beanstalk_environment" "netrox-bean-prod" {
  ...
  setting {
    namespace = "aws:autoscaling:launchconfiguration"
    name      = "EC2KeyName"
    value     = aws_key_pair.netroxkey.key_name
  }
  ...
  setting {
    namespace = "aws:autoscaling:launchconfiguration"
    name      = "SecurityGroups"
    value     = aws_security_group.netrox-prod-sg.id
  }

  setting {
    namespace = "aws:elbv2:loadbalancer"
    name      = "SecurityGroups"
    value     = aws_security_group.netrox-bean-elb-sg.id
  }

  depends_on = [aws_security_group.netrox-bean-elb-sg, aws_security_group.netrox-prod-sg]
